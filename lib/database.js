...
export async function applyBaseMenuToClients(weekId, baseMenus, confirmedClients, assignments) {
  ...
  // Build lookup for base menus by index
  ...
  for (const { client, date } of confirmedClients) {
    try {
      // Check if client already has menu rows for this week
      const { data: existing } = await supabase
        .from('menus')
        .select('id')
        .eq('client_id', client.id)
      ...
      if (existing && existing.length > 0) {
        console.log('[applyBaseMenu] skipping', client.name, '- already has menus');
        results.skipped++;
        continue;
      }

      // Determine assigned meals (override or default)
      const mealsPerWeek = client.meals_per_week || client.mealsPerWeek || 3;
      ...
      assignedMeals.forEach((baseMealIndex, idx) => {
        ...
        if (!baseMeal) {
          console.warn('[applyBaseMenu] base meal', baseMealIndex, 'not found for', client.name);
          return;
        }
        ...
        menuRows.push({
          ...
          week_id: weekId,
          date: date,
          meal_index: idx + 1, // Sequential for client (1, 2, 3...)
          protein: baseMeal.protein || '',
          veg: baseMeal.veg || '',
          starch: baseMeal.starch || '',
          extras: baseMeal.extras || [],
          ...
          base_meal_index: baseMealIndex // Which base meal it came from
        });
      });
      ...
      if (menuRows.length > 0) {
        ...
        if (error) {
          console.error('[applyBaseMenu] insert error for', client.name, error);
          results.errors.push({ client: client.name, error: error.message });
        } else {
          console.log('[applyBaseMenu] created', menuRows.length, 'menus for', client.name);
          results.created += menuRows.length;
        }
      }
    } catch (err) {
      console.error('[applyBaseMenu] error for', client.name, err);
