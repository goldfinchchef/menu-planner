...
export async function saveRecipe(recipe, category) {
  ...
  if (recipe.ingredients && recipe.ingredients.length > 0) {
    ...
    for (const ing of recipe.ingredients) {
      let ingredientId = ing.id || null;

      // If no ingredient_id, try to lookup or create in master ingredients
      if (!ingredientId && ing.name) {
        const normalizedName = ing.name.trim().toLowerCase();
        console.log('[saveRecipe] Looking up ingredient:', ing.name);

        // Try to find existing ingredient by normalized name
        const { data: existingIng, error: lookupError } = await supabase
          .from('ingredients')
          .select('id, name')
        ...
        if (existingIng) {
          ...
          console.log('[saveRecipe] Creating new ingredient:', ing.name);
          try {
            ...
            if (createError) {
              ...
              if (createError.code === '23505' || createError.message?.includes('unique')) {
                console.error(`[saveRecipe] Duplicate ingredient: "${ing.name}". Try selecting from the master list.`);
                throw new Error(`Duplicate ingredient: "${ing.name}". This ingredient already exists - please select it from the master list instead of typing it manually.`);
              }
              throw createError;
            }
            ...
            // Re-throw with helpful message if unique constraint
            if (createErr.message?.includes('Duplicate ingredient')) {
              throw createErr;
            }
            console.error('[saveRecipe] Failed to create ingredient:', ing.name, createErr);
            throw new Error(`Failed to save ingredient "${ing.name}": ${createErr.message}`);
          }
        }
      }
