  ...
  recipes.forEach(recipe => {
    ...
    grouped[category].push({
      ...
      ingredients: (recipe.recipe_ingredients || []).map(ri => ({
        id: ri.ingredient_id || null, // Include ingredient_id from DB
...
export async function saveRecipe(recipe, category) {
  ...
  if (recipe.ingredients && recipe.ingredients.length > 0) {
    ...
    for (const ing of recipe.ingredients) {
      // Check if we have a valid UUID ingredient_id
      let ingredientId = null;
      if (ing.id && UUID_REGEX.test(String(ing.id))) {
        ingredientId = ing.id;
        console.log('[saveRecipe] Using existing ingredient_id:', ingredientId, 'for', ing.name);
      }

      // If no valid ingredient_id, try to lookup by name
      if (!ingredientId && ing.name) {
        const trimmedName = ing.name.trim();
        const normalizedForSearch = trimmedName.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ').trim();
        console.log('[saveRecipe] Looking up ingredient:', trimmedName, 'normalized:', normalizedForSearch);

        // Try to find existing ingredient - query by name_normalized if it exists
        let existingIng = null;

        // First try: query by name_normalized column directly
        const { data: foundByNormalized, error: normError } = await supabase
          .from('ingredients')
          .select('id, name, name_normalized')
          .eq('name_normalized', normalizedForSearch)
          .limit(1)
          .maybeSingle();

        if (!normError && foundByNormalized) {
          existingIng = foundByNormalized;
        } else {
          ...
          if (foundByName) {
            existingIng = foundByName;
          } else {
            // Third try: fetch all and match programmatically
            ...
            if (allIngs) {
              existingIng = allIngs.find(i => {
                const iNorm = (i.name_normalized || i.name.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ').trim());
                return iNorm === normalizedForSearch || i.name.toLowerCase() === trimmedName.toLowerCase();
              });
            }
          }
        }
        ...
        if (existingIng) {
          ...
          console.log('[saveRecipe] Found existing ingredient:', existingIng.name, 'id:', ingredientId);
        } else {
          // Create new ingredient in master list
          console.log('[saveRecipe] Creating new ingredient:', trimmedName);
          try {
            ...
            if (createError) {
              // Unique constraint - ingredient exists, fetch all and find it
              if (createError.code === '23505' || createError.message?.includes('unique') || createError.message?.includes('normalized')) {
                console.log(`[saveRecipe] Constraint error for "${trimmedName}", fetching all to find match...`);
                const { data: allIngs } = await supabase
                  .from('ingredients')
                  .select('id, name, name_normalized')
                  .limit(500);
                ...
                if (allIngs) {
                  ...
                  if (match) {
                    ...
                    console.log('[saveRecipe] Found match after constraint error:', match.name, 'id:', ingredientId);
                  } else {
                    // Log all ingredients for debugging
                    console.error('[saveRecipe] Could not find match. Looking for:', normalizedForSearch);
                    console.error('[saveRecipe] Available (first 20):', allIngs.slice(0, 20).map(i => `${i.name} (${i.name_normalized})`).join(', '));
                    throw new Error(`Ingredient "${trimmedName}" exists but couldn't be matched. Please select it from the master list.`);
                  }
                } else {
                  throw new Error(`Duplicate ingredient: "${trimmedName}". Please select it from the master list.`);
                }
              } else {
                throw createError;
              }
              ...
              console.log('[saveRecipe] Created ingredient:', newIng.name, 'id:', ingredientId);
            }
          } catch (createErr) {
            if (createErr.message?.includes('Duplicate') || createErr.message?.includes('already exists') || createErr.message?.includes('couldn\'t be matched')) {
              throw createErr;
            }
            console.error('[saveRecipe] Failed to create ingredient:', trimmedName, createErr);
            throw new Error(`Failed to save ingredient "${trimmedName}": ${createErr.message}`);
          }
        }
      }
    ...
    if (insertError) {
      ...
      // Check if unique constraint violation and identify the ingredient
      if (insertError.code === '23505' || insertError.message?.includes('unique')) {
        throw new Error(`Failed to save recipe ingredients. A duplicate ingredient was detected. Please ensure all ingredients are selected from the master list.`);
      }
      throw insertError;
    }
