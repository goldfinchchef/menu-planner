...
export async function saveMenu(menuItem) {
  ...
  console.log('[saveMenu] Saving menu item:', record);

  // Use upsert with client_name + date as the conflict key
  const { data, error } = await supabase
    .from('menus')
    .upsert(record, { onConflict: 'client_name,date' })
    .select()
    .single();
...
export async function saveAllMenus(menuItems) {
  ...
  for (const item of menuItems) {
    try {
      ...
      errors++;
      console.error('[saveAllMenus] Failed to save item:', item, e);
    }
  }
