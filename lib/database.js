 ...
 * When menus are unapproved/removed, remove the corresponding delivery_stops.
 *
 * @param {string} weekId - The week ID (e.g., '2026-W07')
 * @returns {Promise<{success: boolean, created: number, deleted: number, error?: string}>}
 */
export async function syncDeliveryStopsForWeek(weekId) {
  console.log('[syncDeliveryStops] start weekId:', weekId);

  const { available, error: availError } = await checkSupabaseAvailable();
  if (!available) {
    console.error('[syncDeliveryStops] unavailable -', availError);
    return { success: false, error: availError, created: 0, deleted: 0 };
  }

  try {
    // 1. Fetch all approved menus for this week
    const { data: approvedMenus, error: menusError } = await supabase
      .from('menus')
    ...
    console.log('[syncDeliveryStops] approved menus count:', approvedMenus?.length);

    // 2. Group approved menus by (client_id, date) - one stop per client per date
    const stopKeys = new Map(); // key: 'client_id|date', value: menu data
    (approvedMenus || []).forEach(menu => {
      if (!menu.client_id || !menu.date) return;
      const key = `${menu.client_id}|${menu.date}`;
      if (!stopKeys.has(key)) {
        stopKeys.set(key, {
    ...
    console.log('[syncDeliveryStops] unique client+date combinations:', stopKeys.size);

    // 3. Fetch existing delivery_stops for this week
    ...
    console.log('[syncDeliveryStops] existing stops count:', existingStops?.length);

    // 4. Determine which stops to create/update and which to delete
    const existingKeys = new Set((existingStops || []).map(s => `${s.client_id}|${s.date}`));
    const stopsToUpsert = [];
    const stopsToDelete = [];

    // Stops to create/update: approved menus that need delivery_stops
    stopKeys.forEach((menuData, key) => {
      stopsToUpsert.push({
        client_id: menuData.client_id,
        client_name: menuData.client_name,
        date: menuData.date,
        week_id: menuData.week_id,
        status: 'MENU_PLANNED',
        updated_at: new Date().toISOString()
      });
    });

    // Stops to delete: existing stops with no approved menu
    (existingStops || []).forEach(stop => {
      const key = `${stop.client_id}|${stop.date}`;
      // Only delete MENU_PLANNED stops (not READY_FOR_DELIVERY which are KDS-managed)
      if (!stopKeys.has(key) && stop.status === 'MENU_PLANNED') {
        stopsToDelete.push(stop.id);
      }
    });

    console.log('[syncDeliveryStops] stops to upsert:', stopsToUpsert.length);
    console.log('[syncDeliveryStops] stops to delete:', stopsToDelete.length);

    // 5. Upsert stops (insert or update by client_id + date)
    let createdCount = 0;
    if (stopsToUpsert.length > 0) {
      ...
      if (upsertError) {
        console.error('[syncDeliveryStops] upsert error:', upsertError);
        // Try individual inserts if batch fails
        for (const stop of stopsToUpsert) {
          try {
            await supabase
              .from('delivery_stops')
              .upsert(stop, { onConflict: 'client_id,date' });
            createdCount++;
          } catch (e) {
            console.warn('[syncDeliveryStops] individual upsert failed:', stop.client_name, e);
          }
        }
      } else {
        createdCount = upsertedData?.length || stopsToUpsert.length;
      }
    }

    // 6. Delete stops that no longer have approved menus
    let deletedCount = 0;
    if (stopsToDelete.length > 0) {
      ...
      if (deleteError) {
        console.error('[syncDeliveryStops] delete error:', deleteError);
      } else {
        deletedCount = stopsToDelete.length;
      }
    }

    console.log('[syncDeliveryStops] complete - created:', createdCount, 'deleted:', deletedCount);

    return { success: true, created: createdCount, deleted: deletedCount };

  } catch (err) {
    console.error('[syncDeliveryStops] error:', err);
    return { success: false, error: err.message, created: 0, deleted: 0 };
  }
}

/**
 * Fetch delivery stops for upcoming dates (for Driver Portal)
 * @param {string} zone - Driver's zone
 * @param {number} daysAhead - Number of days ahead to fetch (default 14)
 * @returns {Promise<Array>} Array of delivery stops with client data
 */
export async function fetchDeliveryStopsForZone(zone, daysAhead = 14) {
  console.log('[fetchDeliveryStops] zone:', zone, 'daysAhead:', daysAhead);

  const { available, error: availError } = await checkSupabaseAvailable();
  if (!available) {
    console.error('[fetchDeliveryStops] unavailable -', availError);
    return [];
  }
  ...
  try {
    ...
    // Fetch stops for the date range with client data
    const { data: stops, error } = await supabase
      ...
      .gte('date', today)
      .lte('date', endDateStr)
      .in('status', ['MENU_PLANNED', 'READY_FOR_DELIVERY', 'IN_PROGRESS'])
      .order('date')
      .order('sort_order');

    if (error) throw error;

    console.log('[fetchDeliveryStops] stops fetched:', stops?.length);
    console.log('[fetchDeliveryStops] sample 3:', stops?.slice(0, 3).map(s => ({
      client_id: s.client_id,
      date: s.date,
      status: s.status
    })));

    // Fetch all clients to join data
    const { data: clients, error: clientsError } = await supabase
      .from('clients')
      .select('id, name, display_name, address, phone, zone, notes');

    if (clientsError) throw clientsError;

    console.log('[fetchDeliveryStops] clients fetched:', clients?.length);

    // Create a map for quick lookup
    const clientMap = new Map();
    (clients || []).forEach(c => clientMap.set(c.id, c));

    // Join stops with client data and filter by zone
    const enrichedStops = (stops || []).map(stop => {
      const client = clientMap.get(stop.client_id);
      if (!client) {
        return {
          ...stop,
          displayName: stop.client_name || 'Unknown',
      ...
      return {
        ...
        address: stop.address || client.address || '',
        phone: client.phone || '',
        zone: client.zone || stop.zone,
        notes: client.notes || '',
        missingClient: false
      };
    }).filter(stop => {
      // Filter by zone if specified
      if (zone && stop.zone !== zone) return false;
      return true;
    });

    console.log('[fetchDeliveryStops] enriched stops for zone:', enrichedStops.length);

    return enrichedStops;

  } catch (err) {
    console.error('[fetchDeliveryStops] error:', err);
    return [];
  }
}
