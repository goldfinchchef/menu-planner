...
export async function saveRecipe(recipe, category) {
  ...
  // STEP 1: Validate all ingredients exist in master list BEFORE saving anything
  if (recipe.ingredients && recipe.ingredients.length > 0) {
    // Fetch all master ingredients once
    ...
    if (fetchError) {
      console.error('[saveRecipe] Failed to fetch master ingredients:', fetchError);
      throw new Error('Failed to verify ingredients. Please try again.');
    }

    // Helper to normalize names for matching
    const normalize = (name) => (name || '').trim().toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ').trim();

    // Build lookup maps for fast matching
    const byNormalized = new Map();
    const byNameLower = new Map();
    masterIngredients.forEach(ing => {
      const normKey = ing.name_normalized || normalize(ing.name);
      byNormalized.set(normKey, ing);
      byNameLower.set(ing.name.toLowerCase(), ing);
    });

    // Resolve each ingredient and collect missing ones
    const resolved = [];
    const missing = [];
    ...
    for (const ing of recipe.ingredients) {
      ...
      // First: check if ingredient already has a valid UUID
      if (ing.id && UUID_REGEX.test(String(ing.id))) {
        // Verify it exists in master list
        match = masterIngredients.find(m => m.id === ing.id);
        if (match) {
          resolved.push({ ing, ingredientId: match.id });
          continue;
        }
      }

      // Second: try to match by normalized name
      if (ing.name) {
        const trimmed = ing.name.trim();
        const normalized = normalize(trimmed);

        match = byNormalized.get(normalized) || byNameLower.get(trimmed.toLowerCase());
    ...
    // Log results
    console.log('[saveRecipe] Resolved:', resolved.length, 'Missing:', missing.length);
    if (missing.length > 0) {
      console.error('[saveRecipe] Missing ingredients:', missing.join(', '));
    }

    // BLOCK SAVE if any ingredients are missing from master list
    if (missing.length > 0) {
      throw new Error(`Missing master ingredients: ${missing.join(', ')}. Add them in the Ingredients tab, then retry.`);
    }
    ...
    // Insert new recipe_ingredients with resolved ingredient_ids
    const ingredientsToInsert = resolved.map(({ ing, ingredientId }) => ({
      recipe_id: data.id,
