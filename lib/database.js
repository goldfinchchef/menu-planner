...
export async function saveClient(client) {
  ...
  // Ensure client_portal_data row exists (upsert with defaults if missing)
  const deliveryDates = client.deliveryDates || [];
  await supabase
    .from('client_portal_data')
    .upsert({
      client_name: client.name,
      selected_dates: deliveryDates,
      ingredient_picks: {},
      notes: ''
    }, {
      onConflict: 'client_name',
      ignoreDuplicates: true  // Don't overwrite existing portal data
    });
...
/**
 * Confirm the next N requested dates for a client.
 * Moves dates from delivery_dates (requested) to confirmed_dates (paid/approved).
 * Does NOT create menu rows - that's Menu Builder's responsibility.
 *
 * @param {string} clientId - UUID of the client
 * @param {number} count - Number of dates to confirm (default 4)
 * @param {Object} options - Optional billing info
 * @param {string} options.invoiceLink - HoneyBook invoice link
 * @param {string} options.dueDate - Bill due date (YYYY-MM-DD)
 * @param {string} options.billingNote - Billing notes
 * @returns {Promise<{success: boolean, confirmed?: string[], remaining?: string[], error?: string}>}
 */
export async function confirmNextDates(clientId, count = 4, options = {}) {
  console.log('[confirmDates] start', { clientId, count, options });

  const { available, error: availError } = await checkSupabaseAvailable();
  if (!available) {
    console.error('[confirmDates] blocked -', availError);
    return { success: false, error: availError };
  }
  ...
  try {
    ...
    const { data: client, error: fetchError } = await supabase
      .from('clients')
      .select('id, name, delivery_dates, confirmed_dates, honeybook_link, bill_due_date, billing_notes')
      .eq('id', clientId)
      .single();

    if (fetchError) {
      console.error('[confirmDates] fetch error', fetchError);
      throw fetchError;
    }

    // 2. Get requested dates, sort chronologically
    const requested = (client.delivery_dates || []).sort();
    if (requested.length === 0) {
      return { success: false, error: 'No requested dates to confirm' };
    }

    // 3. Take first N dates
    const toConfirm = requested.slice(0, count);
    const remaining = requested.slice(count);

    // 4. Merge with existing confirmed dates, dedupe, and sort
    const existingConfirmed = client.confirmed_dates || [];
    const newConfirmed = [...new Set([...existingConfirmed, ...toConfirm])].sort();

    // 5. Build update payload
    const updatePayload = {
      delivery_dates: remaining,
      confirmed_dates: newConfirmed
    };

    // Add optional billing info if provided
    if (options.invoiceLink !== undefined) {
      updatePayload.honeybook_link = options.invoiceLink;
    }
    if (options.dueDate !== undefined) {
      updatePayload.bill_due_date = options.dueDate || null;
    }
    if (options.billingNote !== undefined) {
      updatePayload.billing_notes = options.billingNote;
    }

    console.log('[confirmDates] update payload', { clientId, toConfirm, remaining, updatePayload });
    ...
    if (updateError) {
      console.error('[confirmDates] update error', updateError);
      throw updateError;
    }

    console.log('[confirmDates] success', { clientId, confirmed: toConfirm, remaining });
    return {
      success: true,
      confirmed: toConfirm,
      remaining,
      allConfirmed: newConfirmed
    };
  } catch (err) {
    console.error('[confirmDates] error', err);
    return { success: false, error: err.message || 'Failed to confirm dates' };
  }
}
...
export async function fetchRecipes() {
  console.log('[DB] fetchRecipes -> supabase');
...
export async function saveRecipe(recipe, category) {
  console.log('[saveRecipe] Recipe:', recipe.name, 'Category:', category, 'ID:', recipe.id || 'NEW', 'Ingredients:', recipe.ingredients?.length || 0);

  // Helper: save or update the recipe record
  const saveRecipeRecord = async () => {
    const recipeData = {
      name: recipe.name,
      category: category,
      subcategory: recipe.subcategory || null,
    ...
    if (recipe.id) {
      // UPDATE existing recipe by ID (allows category changes)
      console.log('[saveRecipe] Updating existing recipe by ID:', recipe.id);
      const { data, error } = await supabase
        .from('recipes')
        .update(recipeData)
        .eq('id', recipe.id)
      ...
      if (error) {
        console.error('[saveRecipe] update error:', error);
        throw error;
      }
      console.log('[saveRecipe] Recipe updated, id:', data.id);
      return data;
    } else {
      // INSERT new recipe
      console.log('[saveRecipe] Inserting new recipe');
      const { data, error } = await supabase
        .from('recipes')
        .insert(recipeData)
        .select()
      ...
      if (error) {
        console.error('[saveRecipe] insert error:', error);
        throw error;
      }
      console.log('[saveRecipe] Recipe inserted, id:', data.id);
      return data;
    }
  };
  ...
  if (recipe.ingredients && recipe.ingredients.length > 0) {
    ...
    const data = await saveRecipeRecord();
...
export async function fetchIngredients() {
  console.log('[DB] fetchIngredients -> supabase');
  const { data, error } = await supabase
    .from('ingredients')
    .select('*')
    .order('name');
  ...
  console.log('[DB] fetchIngredients returned:', result.length, 'items');
  return result;
}
...
export async function saveIngredient(ingredient) {
  ...
  // Check if this is an existing ingredient (has valid UUID)
  const isExisting = ingredient.id && UUID_REGEX.test(String(ingredient.id));
  ...
  if (isExisting) {
    ...
    // New ingredient - check if normalized name already exists
    ...
    if (existing) {
      // Update existing ingredient that matches normalized name
      ...
      // Insert new ingredient (no id, let DB generate it)
      const { data, error } = await supabase
        .from('ingredients')
        .insert(payload)
        .select()
...
export async function fetchMenus() {
  ...
  return data.map(m => ({
    id: m.id,
    clientId: m.client_id,       // Include client_id for data integrity
    ...
    baseMealIndex: m.base_meal_index || null,
    inheritedFromBase: m.inherited_from_base || false
  }));
}
...
export async function saveMenu(menuItem, mealIndex = 1, weekId = null) {
  // CRITICAL: Validate client_id is present - database requires NOT NULL
  if (!menuItem.clientId) {
    ...
    console.error('[saveMenu] ❌ BLOCKED - Missing client_id in menu save:', errorPayload);
    throw new Error(`Cannot save menu: client_id is required. clientName=${menuItem.clientName}, date=${menuItem.date}`);
  }
  ...
  // Build the record - include client_id (required) and client_name
  const record = {
    client_id: menuItem.clientId,
  ...
  console.log('[saveMenu] Upserting:', { client_id: record.client_id, client: record.client_name, date: record.date, meal_index: mealIndex, approved: record.approved });

  // Debug log for upsert constraint values
  console.log('[UPSERT DEBUG]', { client_id: record.client_id, date: record.date, meal_index: record.meal_index });

  // Upsert requires UNIQUE constraint on (client_id, date, meal_index) in the database
  const { data, error } = await supabase
    .from('menus')
    .upsert(record, { onConflict: 'client_id,date,meal_index' })
    .select()
    .single();
...
/**
 * Approve ALL menus for a week directly in Supabase
 * Updates all rows where week_id matches and approved=false
 * Database is the single source of truth - no UI filtering
 */
export async function approveAllMenusForWeek(weekId) {
  console.log('[APPROVE ALL] Starting for week:', weekId);
  ...
  console.log('[APPROVE ALL] Success, rows updated:', data?.length || 0);
  return { updated: data?.length || 0, rows: data };
}

/**
 * Delete ALL menu rows for a specific client on a specific date (Start Over)
 * @param {Object} params - { weekId, date, clientId }
 * @returns {Promise<{ deleted: number }>}
 */
export async function deleteMenusForClientDate({ weekId, date, clientId }) {
  console.log('[deleteMenusForClientDate] start', { weekId, date, clientId });

  if (!clientId || !date) {
    throw new Error('clientId and date are required');
  }
  ...
  if (error) {
    console.error('[deleteMenusForClientDate] error:', error);
    throw error;
  }

  console.log('[deleteMenusForClientDate] deleted', data?.length || 0, 'rows');
  return { deleted: data?.length || 0 };
}

/**
 * Create blank menu slots for a client on a specific date
 * Creates N rows with meal_index 1..N, all dishes empty, approved=false
 * @param {Object} params - { weekId, date, clientId, clientName, requiredMeals, portions }
 * @returns {Promise<{ created: number }>}
 */
export async function createBlankMenusForClientDate({ weekId, date, clientId, clientName, requiredMeals, portions = 1 }) {
  console.log('[createBlankMenusForClientDate] start', { weekId, date, clientId, clientName, requiredMeals });

  if (!clientId || !clientName || !date || !weekId) {
    throw new Error('clientId, clientName, date, and weekId are required');
  }

  if (!requiredMeals || requiredMeals < 1) {
    throw new Error('requiredMeals must be >= 1');
  }

  const rows = [];
  for (let i = 1; i <= requiredMeals; i++) {
    rows.push({
      client_id: clientId,
  ...
  if (error) {
    console.error('[createBlankMenusForClientDate] error:', error);
    throw error;
  }

  console.log('[createBlankMenusForClientDate] created', data?.length || 0, 'rows');
  return { created: data?.length || 0 };
}

/**
 * Delete a specific menu row by client, date, and meal_index
 * Used for removing bonus meals
 * @param {Object} params - { clientId, date, mealIndex }
 * @returns {Promise<{ deleted: boolean }>}
 */
export async function deleteMenuRow({ clientId, date, mealIndex }) {
  console.log('[deleteMenuRow] start', { clientId, date, mealIndex });

  if (!clientId || !date || !mealIndex) {
    throw new Error('clientId, date, and mealIndex are required');
  }
  ...
  if (error) {
    console.error('[deleteMenuRow] error:', error);
    throw error;
  }

  console.log('[deleteMenuRow] deleted', data?.length || 0, 'rows');
  return { deleted: (data?.length || 0) > 0 };
}

/**
 * Fetch menus for a specific client and date
 * Returns all meal rows ordered by meal_index
 * @param {Object} params - { clientId, date }
 * @returns {Promise<Array>}
 */
export async function fetchMenusForClientDate({ clientId, date }) {
  console.log('[fetchMenusForClientDate] start', { clientId, date });
  ...
  if (error) {
    console.error('[fetchMenusForClientDate] error:', error);
    throw error;
  }
...
/**
 * Check if all required meal slots are filled for a client/date
 * A meal is "filled" if it has at least one non-empty dish (protein, veg, or starch)
 * @param {Object} params - { clientId, date, requiredMeals }
 * @returns {Promise<{ allFilled: boolean, filledCount: number, requiredMeals: number }>}
 */
export async function checkRequiredMealsFilled({ clientId, date, requiredMeals }) {
  const menus = await fetchMenusForClientDate({ clientId, date });

  // Count meals with at least one filled dish
  let filledCount = 0;
  for (const menu of menus) {
    const hasDish = menu.protein || menu.veg || menu.starch;
    if (hasDish && menu.mealIndex <= requiredMeals) {
      filledCount++;
    }
  }

  return {
    allFilled: filledCount >= requiredMeals,
    filledCount,
    requiredMeals
  };
}
...
/**
 * Get count of unapproved menus for a week (for warning display)
 * Returns { count, byClient: { clientName: count, ... } }
 */
export async function getUnapprovedMenuCountForWeek(weekId) {
  if (!weekId) {
  ...
  if (error) {
    console.error('[getUnapprovedMenuCountForWeek] Error:', error);
    return { count: 0, byClient: {}, error: error.message };
  }

  // Group by client_name for breakdown
  const byClient = {};
...
function transformClientFromDB(dbClient) {
  return {
    ...
    confirmedDates: dbClient.confirmed_dates || [],
...
function transformClientToDB(client) {
  return {
    ...
    confirmed_dates: client.confirmedDates || [],
...
/**
 * Fetch all billing cycles, sorted newest first
 */
export async function fetchBillingCycles() {
  ...
  if (error) {
    console.error('[BillingCycles] fetch error:', error);
    throw error;
  }
...
/**
 * Fetch billing cycles for a specific client
 */
export async function fetchBillingCyclesForClient(clientId) {
  const { data, error } = await supabase
    .from('billing_cycles')
    .select('*')
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[BillingCycles] fetch for client error:', error);
    throw error;
  }
...
/**
 * Fetch approved menus for billing cycle grouping
 */
export async function fetchApprovedMenusForBilling() {
  const { data, error } = await supabase
    .from('menus')
    .select('*')
    .eq('approved', true)
  ...
  if (error) {
    console.error('[BillingCycles] fetch approved menus error:', error);
    throw error;
  }
...
/**
 * Update billing cycle with invoice data
 */
export async function updateBillingCycleInvoice(cycleId, invoiceData) {
  const { data, error } = await supabase
    .from('billing_cycles')
    .update({
      grocery_cost: invoiceData.groceryCost,
  ...
  if (error) {
    console.error('[BillingCycles] update invoice error:', error);
    throw error;
  }
 ...
 */
export async function updateBillingCycleStatus(cycleId, status) {
  const updateData = {
    status,
    updated_at: new Date().toISOString()
  };
  ...
  if (error) {
    console.error('[BillingCycles] update status error:', error);
    throw error;
  }
...
// ============================================================
// SCHEDULE MENUS (for experimental TimelineView)
// ============================================================

/**
 * Fetch menus for a range of week_ids (for schedule grid)
 * Returns all menus regardless of approved status
 */
export async function fetchMenusForWeekRange(weekIds) {
  ...
  const { data, error } = await supabase
    .from('menus')
    .select('id, client_id, client_name, week_id, date, protein, veg, starch, extras, portions, approved, status, meal_index, inherited_from_base, base_meal_index')
    .in('week_id', weekIds)
    .order('date', { ascending: true });

  if (error) {
    console.error('[ScheduleMenus] fetch error:', error);
    throw error;
  }
...
/**
 * Create or update a scheduled menu row for a client + week
 * Creates an empty menu (no protein/veg/starch) when scheduling
 * Uses check-then-insert to prevent duplicates
 */
export async function upsertScheduledMenu({ clientId, clientName, weekId, date, portions }) {
  // First check if a menu already exists for this client + week
  ...
  if (checkError) {
    console.error('[ScheduleMenus] check existing error:', checkError);
    throw checkError;
  }

  // If already exists, return the existing record (don't duplicate)
  if (existing) {
    console.log('[ScheduleMenus] menu already exists for', clientId, weekId);
    return { data: existing, created: false };
  }

  // Insert new empty menu row with status='unconfirmed'
  const { data, error } = await supabase
    .from('menus')
    .insert({
      client_id: clientId,
  ...
  if (error) {
    console.error('[ScheduleMenus] insert error:', error);
    throw error;
  }

  console.log('[ScheduleMenus] created new menu for', clientName, weekId);
  return { data, created: true };
}

/**
 * Delete a scheduled menu row (unschedule a client from a week)
 */
export async function deleteScheduledMenu({ clientId, weekId }) {
  const { error } = await supabase
    .from('menus')
    .delete()
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[ScheduleMenus] delete error:', error);
    throw error;
  }

  console.log('[ScheduleMenus] deleted menu for', clientId, weekId);
  return { success: true };
}

/**
 * Update status for all menus matching client_id + week_id
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID (e.g., "2026-W12")
 * @param {string} status - New status ('unconfirmed', 'confirmed', or 'skipped')
 */
export async function updateMenusStatus({ clientId, weekId, status }) {
  const { data, error } = await supabase
    .from('menus')
    .update({ status })
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[ScheduleMenus] update status error:', error);
    throw error;
  }

  console.log('[ScheduleMenus] updated status to', status, 'for', clientId, weekId, '- rows:', data?.length);
  return { success: true, data, count: data?.length || 0 };
}

// ============================================================
// CLIENT WEEK STATUS (Planning Intent)
// ============================================================

/**
 * Fetch client_week_status rows for visible weeks
 * @param {string[]} weekIds - Array of week IDs to fetch
 * @returns {Promise<Array>} Array of status records
 */
export async function fetchClientWeekStatuses(weekIds) {
  if (!weekIds || weekIds.length === 0) {
    return [];
  }

  const { data, error } = await supabase
    .from('client_week_status')
    .select('id, client_id, week_id, status, created_at, updated_at')
    .in('week_id', weekIds);

  if (error) {
    console.error('[ClientWeekStatus] fetch error:', error);
    throw error;
  }
...
/**
 * Upsert a client_week_status row (for unconfirmed or skipped)
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 * @param {string} status - 'unconfirmed' or 'skipped'
 */
export async function upsertClientWeekStatus({ clientId, weekId, status }) {
  if (status !== 'unconfirmed' && status !== 'skipped') {
    throw new Error(`Invalid status for client_week_status: ${status}`);
  }
  ...
  if (error) {
    console.error('[ClientWeekStatus] upsert error:', error);
    throw error;
  }

  console.log('[ClientWeekStatus] upserted', status, 'for', clientId, weekId);
  return { success: true, data };
}

/**
 * Delete a client_week_status row
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 */
export async function deleteClientWeekStatus({ clientId, weekId }) {
  const { error } = await supabase
    .from('client_week_status')
    .delete()
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[ClientWeekStatus] delete error:', error);
    throw error;
  }

  console.log('[ClientWeekStatus] deleted for', clientId, weekId);
  return { success: true };
}

/**
 * Confirm a client week by creating all menu rows
 * Creates one row per meal based on client.meals_per_week
 * @param {Object} client - Client object with id, name, meals_per_week, portions
 * @param {string} weekId - Week ID
 * @param {string} weekStartDate - Date string (YYYY-MM-DD) for week start
 */
export async function confirmClientWeek({ client, weekId, weekStartDate }) {
  const mealsPerWeek = client.meals_per_week || client.mealsPerWeek || 3;
  const portions = client.portions || 4;

  // First, delete any existing client_week_status row
  await supabase
    .from('client_week_status')
    .delete()
  ...
  // Check if menu rows already exist
  const { data: existing } = await supabase
    .from('menus')
    .select('id')
    .eq('client_id', client.id)
  ...
  if (existing && existing.length > 0) {
    console.log('[ConfirmClientWeek] menus already exist for', client.id, weekId);
    return { success: true, created: 0, existed: true };
  }

  // Create menu rows for each meal slot
  const menuRows = [];
  for (let mealIndex = 1; mealIndex <= mealsPerWeek; mealIndex++) {
    menuRows.push({
      client_id: client.id,
      client_name: client.name,
      week_id: weekId,
  ...
  if (error) {
    console.error('[ConfirmClientWeek] insert error:', error);
    throw error;
  }

  console.log('[ConfirmClientWeek] created', data?.length, 'menu rows for', client.name, weekId);
  return { success: true, created: data?.length || 0 };
}

/**
 * Unconfirm a client week by deleting all menu rows
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 * @returns {Object} { deleted: number, hadContent: boolean }
 */
export async function deleteMenusForClientWeek({ clientId, weekId }) {
  // First check what we're deleting
  const { data: existing } = await supabase
    .from('menus')
    .select('id, protein, veg, starch')
    .eq('client_id', clientId)
    .eq('week_id', weekId);

  const hadContent = existing?.some(m => m.protein || m.veg || m.starch) || false;

  // Delete all menu rows for this client/week
  const { error } = await supabase
    .from('menus')
    .delete()
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[DeleteMenusForClientWeek] error:', error);
    throw error;
  }

  console.log('[DeleteMenusForClientWeek] deleted', existing?.length || 0, 'rows for', clientId, weekId);
  return { success: true, deleted: existing?.length || 0, hadContent };
}

/**
 * Clear ALL client menus for a week (Reset Week)
 * Deletes from `menus` table only - preserves base_weekly_menus and client_meal_assignments
 * @param {string} weekId - Week ID (e.g., "2026-W18")
 * @returns {Object} { success: boolean, deleted: number }
 */
export async function clearWeekMenus(weekId) {
  if (!weekId) {
    throw new Error('weekId is required');
  }

  console.log('[clearWeekMenus] Clearing all client menus for week:', weekId);
  ...
  if (countError) {
    console.error('[clearWeekMenus] count error:', countError);
    throw countError;
  }

  const count = existing?.length || 0;
  console.log('[clearWeekMenus] Found', count, 'menu rows to delete');

  if (count === 0) {
    return { success: true, deleted: 0 };
  }

  // Delete all menu rows for this week
  const { error: deleteError } = await supabase
    .from('menus')
    .delete()
    .eq('week_id', weekId);
  ...
  if (deleteError) {
    console.error('[clearWeekMenus] delete error:', deleteError);
    throw deleteError;
  }

  console.log('[clearWeekMenus] Deleted', count, 'menu rows for week', weekId);
...
/**
 * Fetch base weekly menus for a specific week
 * @param {string} weekId - Week ID (e.g., "2026-W18")
 * @returns {Array} Array of 4 base meals (or fewer if not all created)
 */
export async function fetchBaseWeeklyMenus(weekId) {
  if (!weekId) return [];

  const { data, error } = await supabase
    .from('base_weekly_menus')
    .select('*')
    .eq('week_id', weekId)
    .order('meal_index', { ascending: true });

  if (error) {
    console.error('[fetchBaseWeeklyMenus] error:', error);
    throw error;
  }
...
/**
 * Save a base weekly menu item (upsert by week_id + meal_index)
 * @param {Object} baseMeal - { weekId, mealIndex, protein, veg, starch, extras }
 */
export async function saveBaseWeeklyMenu(baseMeal) {
  const record = {
    week_id: baseMeal.weekId,
    meal_index: baseMeal.mealIndex,
    protein: baseMeal.protein || '',
    veg: baseMeal.veg || '',
    starch: baseMeal.starch || '',
    extras: baseMeal.extras || []
  };

  const { data, error } = await supabase
    .from('base_weekly_menus')
    .upsert(record, { onConflict: 'week_id,meal_index' })
    .select()
    .single();

  if (error) {
    console.error('[saveBaseWeeklyMenu] error:', error);
    throw error;
  }

  console.log('[saveBaseWeeklyMenu] saved meal', baseMeal.mealIndex, 'for week', baseMeal.weekId);
  return data;
}

/**
 * Save all 4 base weekly menus at once
 * @param {string} weekId - Week ID
 * @param {Array} meals - Array of 4 meal objects [{ protein, veg, starch, extras }, ...]
 */
export async function saveAllBaseWeeklyMenus(weekId, meals) {
  const records = meals.map((meal, idx) => ({
    week_id: weekId,
    meal_index: idx + 1,
  ...
  const { data, error } = await supabase
    .from('base_weekly_menus')
    .upsert(records, { onConflict: 'week_id,meal_index' })
    .select();

  if (error) {
    console.error('[saveAllBaseWeeklyMenus] error:', error);
    throw error;
  }

  console.log('[saveAllBaseWeeklyMenus] saved', data.length, 'meals for week', weekId);
  return data;
}

// ============ CLIENT MEAL ASSIGNMENTS ============

/**
 * Fetch meal assignment override for a client/week
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 * @returns {Object|null} Assignment record or null if using default
 */
export async function fetchClientMealAssignment(clientId, weekId) {
  if (!clientId || !weekId) return null;
  ...
  if (error && error.code !== 'PGRST116') { // PGRST116 = no rows found
    console.error('[fetchClientMealAssignment] error:', error);
    throw error;
  }
...
/**
 * Fetch all meal assignments for a week
 * @param {string} weekId - Week ID
 * @returns {Array} Array of assignment records
 */
export async function fetchAllClientMealAssignments(weekId) {
  if (!weekId) return [];
  ...
  if (error) {
    console.error('[fetchAllClientMealAssignments] error:', error);
    throw error;
  }
...
/**
 * Save a client meal assignment override
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 * @param {Array} assignedMeals - Array of meal indices, e.g., [1, 4]
 */
export async function saveClientMealAssignment(clientId, weekId, assignedMeals) {
  const record = {
    client_id: clientId,
    week_id: weekId,
  ...
  const { data, error } = await supabase
    .from('client_meal_assignments')
    .upsert(record, { onConflict: 'client_id,week_id' })
    .select()
    .single();

  if (error) {
    console.error('[saveClientMealAssignment] error:', error);
    throw error;
  }

  console.log('[saveClientMealAssignment] saved', assignedMeals, 'for client', clientId, 'week', weekId);
  return data;
}

/**
 * Delete a client meal assignment (revert to default)
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 */
export async function deleteClientMealAssignment(clientId, weekId) {
  const { error } = await supabase
    .from('client_meal_assignments')
    .delete()
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[deleteClientMealAssignment] error:', error);
    throw error;
  }

  console.log('[deleteClientMealAssignment] deleted override for client', clientId, 'week', weekId);
}

/**
 * Get default meal assignment based on meals_per_week
 * @param {number} mealsPerWeek - Number of meals (1-4)
 * @returns {Array} Default meal indices, e.g., [1, 2] for 2-meal plan
 */
export function getDefaultMealAssignment(mealsPerWeek) {
  const count = Math.min(Math.max(mealsPerWeek || 1, 1), 4);
  return Array.from({ length: count }, (_, i) => i + 1);
}

/**
 * Apply base weekly menu to all confirmed clients for a week
 * Creates menu rows with inheritance tracking
 *
 * @param {string} weekId - Week ID
 * @param {Array} baseMenus - Array of base menu objects from fetchBaseWeeklyMenus
 * @param {Array} confirmedClients - Array of { client, date } for confirmed clients
 * @param {Array} assignments - Array of meal assignment overrides from fetchAllClientMealAssignments
 * @returns {Object} { created: number, skipped: number, errors: [] }
 */
export async function applyBaseMenuToClients(weekId, baseMenus, confirmedClients, assignments) {
  /**
   * Simplified Apply Base Menu (experimental workflow)
   *
   * For each confirmed client:
   * 1. DELETE all existing menu rows for client + week (no inspection)
   * 2. CREATE fresh menu rows from base_weekly_menus + assignments
   *
   * This eliminates edge cases with empty shells, partial data, stale rows.
   */
  const results = {
    success: true,
    regenerated: 0,      // clients processed
    deleted: 0,          // total rows deleted
    created: 0,          // total rows created
    errors: []
  };

  console.log('[applyBaseMenu] === Apply Base Menu (Clean Regenerate) ===');
  console.log('[applyBaseMenu] weekId:', weekId);
  console.log('[applyBaseMenu] baseMenus:', baseMenus.length);
  console.log('[applyBaseMenu] confirmedClients:', confirmedClients.map(c => c.client.name));

  // Build lookup for base menus by meal_index
  const baseMenuByIndex = {};
  baseMenus.forEach(m => {
    baseMenuByIndex[m.meal_index] = m;
  });

  // Build lookup for assignments by client_id
  const assignmentByClientId = {};
  assignments.forEach(a => {
    assignmentByClientId[a.client_id] = a.assigned_meals;
  });

  for (const { client, date } of confirmedClients) {
    try {
      // STEP 1: Delete ALL existing menu rows for this client + week
      ...
      if (deleteError) {
        console.error('[applyBaseMenu] DELETE error for', client.name, deleteError);
        results.errors.push({ client: client.name, error: `Delete: ${deleteError.message}` });
        continue;
      }

      const deletedCount = deletedRows?.length || 0;
      results.deleted += deletedCount;
      console.log('[applyBaseMenu]', client.name, '- deleted', deletedCount, 'existing rows');

      // STEP 2: Create fresh menu rows from base menus
      const mealsPerWeek = client.meals_per_week || client.mealsPerWeek || 3;
      const assignedMeals = assignmentByClientId[client.id] || getDefaultMealAssignment(mealsPerWeek);

      const menuRows = [];
      assignedMeals.forEach((baseMealIndex, idx) => {
        const baseMeal = baseMenuByIndex[baseMealIndex];
        if (!baseMeal) {
          console.warn('[applyBaseMenu]', client.name, '- base meal', baseMealIndex, 'not found');
          return;
        }
        ...
        menuRows.push({
          ...
          base_meal_index: baseMealIndex
        });
      });

      if (menuRows.length === 0) {
        console.warn('[applyBaseMenu]', client.name, '- no rows generated (missing base meals?)');
        results.errors.push({ client: client.name, error: 'No rows generated - check base menus' });
        continue;
      }
      ...
      if (insertError) {
        console.error('[applyBaseMenu] INSERT error for', client.name, insertError);
        results.errors.push({ client: client.name, error: `Insert: ${insertError.message}` });
        continue;
      }
      ...
      results.regenerated++;
      console.log('[applyBaseMenu]', client.name, '- inserted', insertedCount, 'fresh rows');

    } catch (err) {
      console.error('[applyBaseMenu] Exception for', client.name, err);
      results.errors.push({ client: client.name, error: err.message });
    }
  }

  console.log('[applyBaseMenu] === Results ===');
  console.log('[applyBaseMenu] Regenerated:', results.regenerated, 'clients');
  console.log('[applyBaseMenu] Deleted:', results.deleted, 'rows');
  console.log('[applyBaseMenu] Created:', results.created, 'rows');
  console.log('[applyBaseMenu] Errors:', results.errors.length);

  return results;
}

/**
 * Rebuild a single client's menus based on their current meal assignment.
 * Does NOT affect other clients.
 * @param {string} weekId - The week ID
 * @param {Array} baseMenus - Array of base_weekly_menus rows
 * @param {Object} client - Client object with id, name, meals_per_week, portions
 * @param {string} deliveryDate - The delivery date for this client
 * @param {Array} assignedMeals - Array of meal indices like [1, 3, 4]
 * @returns {Object} { success, deleted, created, error }
 */
export async function rebuildSingleClientMenus(weekId, baseMenus, client, deliveryDate, assignedMeals) {
  const result = { success: false, deleted: 0, created: 0, error: null };

  console.log('[rebuildSingleClient]', client.name, '- rebuilding with assignment:', assignedMeals);
  ...
  try {
    // STEP 1: Delete existing menu rows for this client + week
    const { data: deletedRows, error: deleteError } = await supabase
      .from('menus')
      .delete()
      .eq('client_id', client.id)
    ...
    if (deleteError) {
      console.error('[rebuildSingleClient] DELETE error:', deleteError);
      result.error = `Delete failed: ${deleteError.message}`;
      return result;
    }

    result.deleted = deletedRows?.length || 0;
    console.log('[rebuildSingleClient]', client.name, '- deleted', result.deleted, 'existing rows');

    // STEP 2: Create fresh menu rows from base menus + assignment
    const menuRows = [];
    assignedMeals.forEach((baseMealIndex, idx) => {
      const baseMeal = baseMenuByIndex[baseMealIndex];
      if (!baseMeal) {
        console.warn('[rebuildSingleClient]', client.name, '- base meal', baseMealIndex, 'not found');
        return;
      }
    ...
    if (menuRows.length === 0) {
      result.error = 'No menu rows generated - check base menus';
      return result;
    }
    ...
    if (insertError) {
      console.error('[rebuildSingleClient] INSERT error:', insertError);
    ...
    result.success = true;
    console.log('[rebuildSingleClient]', client.name, '- inserted', result.created, 'rows');

    return result;

  } catch (err) {
    console.error('[rebuildSingleClient] Exception:', err);
...
/**
 * Update a single menu row by ID (for client-level overrides)
 * Sets inherited_from_base = false when manually edited
 * @param {string} menuId - The menu row ID
 * @param {Object} updates - { protein, veg, starch, extras }
 * @returns {Object} Updated menu row
 */
export async function saveClientMeal(menuId, updates) {
  const { data, error } = await supabase
    .from('menus')
    .update({
      protein: updates.protein || '',
      veg: updates.veg || '',
      starch: updates.starch || '',
      extras: updates.extras || [],
      inherited_from_base: false // Mark as manually edited
    })
    .eq('id', menuId)
    .select()
    .single();
  ...
  if (error) {
    console.error('[saveClientMeal] error:', error);
    throw error;
  }

  console.log('[saveClientMeal] updated menu', menuId, '- inherited_from_base = false');
  return data;
}

/**
 * Confirm/approve all menus for a client + week
 * Sets approved = true on all menu rows
 * Does NOT prevent future edits - just a status flag
 * @param {string} clientId - Client ID
 * @param {string} weekId - Week ID
 * @returns {Object} { success, updated, data }
 */
export async function confirmClientMenusForWeek(clientId, weekId) {
  const { data, error } = await supabase
    .from('menus')
    .update({ approved: true })
    .eq('client_id', clientId)
  ...
  if (error) {
    console.error('[confirmClientMenus] error:', error);
    throw error;
  }

  console.log('[confirmClientMenus] approved', data?.length, 'menus for client', clientId, 'week', weekId);
  return { success: true, updated: data?.length || 0, data };
}

/**
 * Fetch component usage history from BASE MENUS for the last N weeks
 * Returns a map of component names to their usage stats
 * Only counts base menus (not per-client menus) - so each component
 * is counted once per week it appears, not once per client.
 * @param {number} weeks - Number of weeks to look back (default 5)
 * @param {string} excludeWeekId - Week ID to exclude (current week being edited)
 * @returns {Object} { componentName: { count, lastWeekId, weeks: Set } }
 */
export async function fetchComponentUsageHistory(weeks = 5, excludeWeekId = null) {
  // Generate week IDs for the lookback period, excluding current week
  const weekIds = [];
  const now = new Date();
  for (let i = 0; i < weeks + 1 && weekIds.length < weeks; i++) {
    ...
    // Skip the excluded week
    if (weekId !== excludeWeekId) {
      weekIds.push(weekId);
    }
  }
  ...
  const { data, error } = await supabase
    .from('base_weekly_menus')
    .select('protein, veg, starch, extras, week_id')
    .in('week_id', weekIds);

  if (error) {
    console.error('[fetchComponentUsageHistory] error:', error);
    return {};
  }

  // Build usage map - count WEEKS used, not individual meal slots
  const usageMap = {};

  const addUsage = (component, weekId) => {
    if (!component || component.trim() === '') return;

    const name = component.trim();
    if (!usageMap[name]) {
      usageMap[name] = { count: 0, lastWeekId: null, weeks: new Set() };
    }

    // Only count once per week
    if (!usageMap[name].weeks.has(weekId)) {
      usageMap[name].weeks.add(weekId);
      usageMap[name].count = usageMap[name].weeks.size;
    }

    // Track most recent usage
    if (!usageMap[name].lastWeekId || weekId > usageMap[name].lastWeekId) {
      usageMap[name].lastWeekId = weekId;
    }
  };

  (data || []).forEach(menu => {
    const { protein, veg, starch, extras, week_id } = menu;

    addUsage(protein, week_id);
    addUsage(veg, week_id);
    addUsage(starch, week_id);

    // Handle extras array
    if (Array.isArray(extras)) {
      extras.forEach(extra => addUsage(extra, week_id));
    }
  });

  // Clean up the weeks Set before returning (not JSON serializable)
  Object.values(usageMap).forEach(usage => {
    delete usage.weeks;
  });

  console.log('[fetchComponentUsageHistory]', Object.keys(usageMap).length, 'components tracked from', data?.length, 'base menu rows');
  return usageMap;
}
