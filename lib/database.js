...
export async function fetchComponentUsageHistory(weeks = 5) {
  ...
  const addUsage = (component, date, weekId) => {
    if (!component || component.trim() === '') return;

    const name = component.trim();
    if (!usageMap[name]) {
      usageMap[name] = { count: 0, lastUsed: null, lastWeekId: null };
    }
    usageMap[name].count++;

    // Track most recent usage
    if (!usageMap[name].lastUsed || date > usageMap[name].lastUsed) {
      usageMap[name].lastUsed = date;
      usageMap[name].lastWeekId = weekId;
    }
  };

  (data || []).forEach(menu => {
    const { protein, veg, starch, extras, date, week_id } = menu;

    addUsage(protein, date, week_id);
    addUsage(veg, date, week_id);
    addUsage(starch, date, week_id);

    // Handle extras array
    if (Array.isArray(extras)) {
      extras.forEach(extra => addUsage(extra, date, week_id));
    }
  });

  console.log('[fetchComponentUsageHistory]', Object.keys(usageMap).length, 'components tracked from', data?.length, 'menus');
  return usageMap;
}
