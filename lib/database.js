  ...
  if (recipe.ingredients && recipe.ingredients.length > 0) {
    ...
    for (const ing of recipe.ingredients) {
      ...
      if (!ingredientId && ing.name) {
        ...
        console.log('[saveRecipe] Looking up ingredient by name:', trimmedName);

        // Query using case-insensitive match - try multiple approaches
        // First try exact match on name_normalized if it exists
        let existingIng = null;

        // Try lookup by normalized name pattern (handles DB's normalization)
        const { data: found, error: lookupError } = await supabase
          .from('ingredients')
          .select('id, name')
          .or(`name.ilike.${trimmedName},name.ilike.${trimmedName.toLowerCase()}`)
          .limit(1)
          .maybeSingle();

        if (lookupError) {
          console.warn('[saveRecipe] ingredient lookup error:', lookupError);
          // Try a simpler query as fallback
          const { data: fallbackFound } = await supabase
            .from('ingredients')
            .select('id, name')
            .ilike('name', `%${trimmedName}%`)
            .limit(1)
            .maybeSingle();
          existingIng = fallbackFound;
        } else {
          existingIng = found;
        }
        ...
        if (existingIng) {
          ...
          try {
            // Use INSERT instead of UPSERT since we don't know the exact conflict target
            ...
            if (createError) {
              // Check if unique constraint violation - ingredient already exists
              if (createError.code === '23505' || createError.message?.includes('unique') || createError.message?.includes('normalized')) {
                console.log(`[saveRecipe] Ingredient "${trimmedName}" exists (constraint), looking up...`);
                // Do another lookup to get the existing ID
                ...
                if (retryFound) {
                  ingredientId = retryFound.id;
                  console.log('[saveRecipe] Found on retry:', retryFound.name, 'id:', ingredientId);
                } else {
                  // Last resort - try broader search
                  const { data: broadFound } = await supabase
                    .from('ingredients')
                    .select('id, name')
                    .ilike('name', `%${trimmedName.split(' ')[0]}%`)
                    .limit(5);

                  if (broadFound && broadFound.length > 0) {
                    // Find best match
                    const exactMatch = broadFound.find(i => i.name.toLowerCase() === trimmedName.toLowerCase());
                    if (exactMatch) {
                      ingredientId = exactMatch.id;
                      console.log('[saveRecipe] Found exact match on broad search:', exactMatch.name);
                    } else {
                      console.error(`[saveRecipe] Could not find ingredient "${trimmedName}" after constraint error. Similar: ${broadFound.map(i => i.name).join(', ')}`);
                      throw new Error(`Ingredient "${trimmedName}" already exists but couldn't be matched. Similar ingredients: ${broadFound.map(i => i.name).join(', ')}. Please select from the master list.`);
                    }
                  } else {
