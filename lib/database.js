...
export async function saveDeliveryRoute(weekId, date, zone, stops, extraFields = {}) {
  // IMMEDIATE LOG when function is called
  console.log('[DB FUNCTION CALLED] saveDeliveryRoute', { weekId, date, zone, stopsCount: stops?.length, hasSupabase: !!supabase });

  if (!supabase) {
    console.error('[DB ERROR] Supabase client is NULL');
    throw new Error('Supabase client not initialized. Check environment variables.');
  }

  if (!weekId || !date || !zone) {
    console.error('[DB ERROR] Missing required fields', { weekId, date, zone });
    throw new Error(`Missing required fields: week_id=${weekId}, date=${date}, zone=${zone}`);
  }

  // Build payload for delivery_runs table
  const payload = {
    week_id: weekId,
    date: date,
    zone: zone,
  ...
  // LOG THE FULL PAYLOAD before writing
  console.log('[DB WRITE] delivery_runs payload:', payload);

  // Upsert using unique constraint (week_id, date, zone)
  ...
  if (runError) {
    console.error('[DB ERROR] Upsert failed:', runError);
    throw new Error(`Failed to save route: ${runError.message}`);
  }

  console.log('[DB SUCCESS] Saved to delivery_runs, id:', runData.id);
  return runData.id;
}
