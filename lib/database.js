...
export async function saveClientToSupabase(client) {
  console.log('[saveClient] start', client.name);
  console.log('[saveClient] payload (id present?):', !!client.id, 'id:', client.id);
  ...
  try {
    ...
    console.log('[saveClient] success', client.name);
...
export async function saveRecipeToSupabase(recipe, category) {
  console.log('[saveRecipe] start', recipe.name, 'category:', category);
  console.log('[saveRecipe] payload - ingredients count:', recipe.ingredients?.length || 0);

  const { available, error: availError } = await checkSupabaseAvailable();
  if (!available) {
    console.error('[saveRecipe] blocked -', availError);
    return { success: false, error: availError };
  }

  try {
    await saveRecipe(recipe, category);
    console.log('[saveRecipe] success', recipe.name);

    const recipes = await fetchRecipes();
    const totalRecipes = Object.values(recipes).flat().length;
    console.log('[saveRecipe] refetch count:', totalRecipes);
...
export async function saveMenu(menuItem, mealIndex = 1, weekId = null) {
  ...
  // This is also done by a database trigger, but we set it explicitly for clarity
  ...
  console.log('[saveMenu] Saving menu item (week_id=' + resolvedWeekId + ', meal_index=' + mealIndex + '):', record);

  // Use upsert with client_name + date + meal_index as the conflict key
  const { data, error } = await supabase
    .from('menus')
