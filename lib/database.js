...
export async function saveRecipeToSupabase(recipe, category) {
  console.log('[saveRecipe] start', recipe?.name || 'undefined');

  // Validate recipe object
  if (!recipe) {
    console.error('[saveRecipe] blocked - recipe is undefined');
    return { success: false, error: 'Recipe data is missing' };
  }

  // Validate required fields
  if (!recipe.name || recipe.name.trim() === '') {
    console.error('[saveRecipe] blocked - recipe name is empty');
    return { success: false, error: 'Recipe name is required' };
  }

  if (!category || category.trim() === '') {
    console.error('[saveRecipe] blocked - category is empty');
    return { success: false, error: 'Recipe category is required' };
  }

  // Log payload details
  console.log('[saveRecipe] payload', JSON.stringify({
    name: recipe.name,
    category: category,
