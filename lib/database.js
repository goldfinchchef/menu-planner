 ...
 * Uses Supabase join to get client addresses in a single query.
 *
...
export async function fetchDeliveryStopsForZone(zone, daysAhead = 14) {
  console.log('[DriverPortal] fetchDeliveryStops zone:', zone, 'daysAhead:', daysAhead);

  const { available, error: availError } = await checkSupabaseAvailable();
  if (!available) {
    console.error('[DriverPortal] unavailable -', availError);
    return [];
  }
  ...
  try {
    ...
    // Fetch stops with client data using Supabase join syntax
    ...
    console.log('[DriverPortal] stops count:', stops?.length);
    console.log('[DriverPortal] sample:', stops?.slice(0, 3));

    // Transform stops with client data
    const enrichedStops = (stops || []).map(stop => {
      const client = stop.clients;
      if (!client) {
        return {
          ...stop,
    ...
    console.log('[DriverPortal] enriched stops for zone:', enrichedStops.length);

    return enrichedStops;

  } catch (err) {
    console.error('[DriverPortal] fetchDeliveryStops error:', err);
    return [];
  }
}
