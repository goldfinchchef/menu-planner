...
export async function syncToSupabase(localData) {
  ...
  try {
    // NOTE: Drivers and Menus are NOT synced here - they are saved directly via explicit actions
    // This prevents duplicate writes and sync conflicts
    ...
    // Save all entities (except drivers and menus)
    await Promise.all([
