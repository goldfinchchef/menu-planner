...
export async function syncToSupabase(localData) {
  ...
  try {
    // Pre-sanitize drivers to remove invalid IDs before sync
    let sanitizedDrivers = localData.drivers || [];
    if (sanitizedDrivers.length > 0) {
      console.log('[syncDrivers] Starting sync for', sanitizedDrivers.length, 'drivers');

      // Log sample of what we're sending
      const sample = sanitizedDrivers[0];
      console.log('[syncDrivers] Sample driver before sanitize:', JSON.stringify({
        id: sample?.id,
        idType: typeof sample?.id,
        name: sample?.name,
        isValidUUID: isValidUUID(sample?.id)
      }));

      // Sanitize: remove invalid IDs so Supabase generates new UUIDs
      sanitizedDrivers = sanitizedDrivers.map(driver => {
        if (driver.id && !isValidUUID(driver.id)) {
          console.log('[syncDrivers] sanitizing invalid id:', driver.id, 'for driver:', driver.name);
          // Return driver without id field
          const { id, ...driverWithoutId } = driver;
          return driverWithoutId;
        }
        return driver;
      });
    }
    ...
    const results = await Promise.all([
      ...
      sanitizedDrivers.length > 0 ? saveAllDrivers(sanitizedDrivers) : Promise.resolve(),
    ...
    // Log driver sync results
    const savedDrivers = results[5];
    if (Array.isArray(savedDrivers) && savedDrivers.length > 0) {
      console.log('[syncDrivers] success count:', savedDrivers.length);
      console.log('[syncDrivers] Sample saved driver:', JSON.stringify(savedDrivers[0]));

      // Update localStorage with real UUIDs from Supabase
      try {
        const currentData = localStorage.getItem(STORAGE_KEY);
        if (currentData) {
          const parsed = JSON.parse(currentData);
          // Create a map of name -> saved driver with real UUID
          const driverMap = {};
          savedDrivers.forEach(d => { driverMap[d.name] = d; });

          // Update drivers in localStorage with real UUIDs
          if (parsed.drivers) {
            parsed.drivers = parsed.drivers.map(d => driverMap[d.name] || d);
            localStorage.setItem(STORAGE_KEY, JSON.stringify(parsed));
            console.log('[syncDrivers] Updated localStorage with real UUIDs');
          }
        }
      } catch (e) {
        console.warn('[syncDrivers] Could not update localStorage:', e);
      }
    }
